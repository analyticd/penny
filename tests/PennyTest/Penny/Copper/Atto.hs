module PennyTest.Penny.Copper.Atto where
...
import qualified Penny.Copper.Atto as C
...
tests :: Test
tests = testGroup "PennyTest.Penny.Copper.Atto"
  [ pTest "lvl1SubAcct"
  ...
  , testProperty "tzSign" $ do
      i <- Q.arbitrary
      (fExp, txt) <- P.tzSign
      let eiFParsed = parseOnly (C.tzSign <* Atto.endOfInput) txt
      case eiFParsed of
        Left e ->
  ...
  , testProperty "blankLine" $ do
      (_, txt) <- P.blankLine
      case parseOnly (C.blankLine <* Atto.endOfInput) txt of
...
pTestByT eq s p g = testProperty s t
  where
    t = Ex.resolveT return $ do
      (r, txt) <- g
      case parseOnly (p <* Atto.endOfInput) txt of
        Left e ->
