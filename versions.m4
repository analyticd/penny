divert(-1)

# Haskell Platform
define(`pv_base', `4.6.0.1')
define(`pv_base_max', `5')
define(`pv_bytestring', `0.10.0.2')
define(`pv_containers', `0.5.0.0')
define(`pv_old_locale', `1.0.0.5')
define(`pv_parsec', `3.1.3')
define(`pv_QuickCheck', `2.5')
define(`pv_split', `0.2.2')
define(`pv_text', `0.11.3.1')
define(`pv_time', `1.4.0.1')
define(`pv_transformers', `0.3.0.0')

# Packages I maintain
define(`pv_anonymous_sums', `0.2.0.0')
define(`pv_anonymous_sums_max', `0.3.0.0')

define(`pv_matchers', `0.14.0.0')
define(`pv_matchers_max', `0.15.0.0')

define(`pv_multiarg', `0.24.0.0')
define(`pv_multiarg_max', `0.25.0.0')

define(`pv_ofx', `0.4.0.0')
define(`pv_ofx_max', `0.5.0.0')

define(`pv_prednote', `0.18.0.0')
define(`pv_prednote_max', `0.19.0.0')

define(`pv_rainbow', `0.6.0.0')
define(`pv_rainbow_max', `0.7.0.0')

# Other packages
define(`pv_action_permutations', `0.0.0.0')
define(`pv_cereal', `0.3.5.2')
define(`pv_either', `3.4.1')
define(`pv_pretty_show', `1.5')
define(`pv_random', `1.0.1.1')
define(`pv_random_shuffle', `0.0.4')
define(`pv_semigroups', `0.9.2')

divert(1)dnl
