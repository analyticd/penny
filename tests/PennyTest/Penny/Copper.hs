...
lvl1AcctChar = G.suchThat (G.oneof [unicodeAll, asciiAll]) A.lvl1Char

-- | Generates Level 1 sub-accounts; however, does not make sure they are valid.
lvl1SubAcct :: Gen L.SubAccount
lvl1SubAcct = (L.SubAccount . X.pack) <$> G.listOf1 lvl1AcctChar

-- | Generates Level 1 accounts; however, does not make sure they are valid.
lvl1Account :: Gen L.Account
lvl1Account = L.Account <$> G.listOf1 lvl1SubAcct

lvl2AcctFirstChar :: Gen Char
lvl2AcctFirstChar = G.oneof [unicodeAll, letter]

lvl2AcctRemainingChar :: Gen Char
lvl2AcctRemainingChar = G.suchThat (G.oneof [unicodeAll, asciiAll])
                        A.lvl2RemainingChar
...
test_parseRenderLvl1 :: Gen P.Result
test_parseRenderLvl1 = parseRender lvl1Account "1"

-- | Parsing a rendered level 2 account should yield same thing
test_parseRenderLvl2 :: Gen P.Result
test_parseRenderLvl2 = parseRender lvl2Account "2"
